...
class MultiViewController: BaseViewController, ReactorKit.View {
  ...
  // MARK: Initializers
  required init(reactor: MultiViewReactor) {
  ...
  func bind(reactor: MultiViewReactor) {
    ...
    nextButton.rx.tap
      .bind { [weak self] in
        let complexViewReactor = ComplexViewReactor()
        let viewController = ComplexViewController(reactor: complexViewReactor)
